...
const COST_DATA = {
  'United States': {
    ...
    symbol: '$',
    items: [
      { category: 'LLC Registration', min: 50, max: 500, note: 'Varies by state. Delaware and Wyoming are common comparison points.' },
      ...
      { category: 'EIN (Tax ID)', min: 0, max: 0, note: 'Often free from IRS where eligible.' },
      { category: 'Bank Account Opening', min: 0, max: 100, note: 'Most banks are free; some require deposit or physical presence.' },
      { category: 'Accountant / CPA', min: 500, max: 2500, note: 'Annual cost; varies by business size.' },
      { category: 'Office / Co-working', min: 200, max: 3000, note: 'Monthly. Major cities are higher.' },
    ],
  },
  'United Kingdom': {
    currency: 'GBP',
    symbol: 'GBP',
    items: [
      { category: 'Ltd Company Registration', min: 12, max: 50, note: 'Online via Companies House. Same-day options exist.' },
      { category: 'Business Bank Account', min: 0, max: 10, note: 'Monthly fee. Several free options available.' },
      { category: 'VAT Registration', min: 0, max: 0, note: 'Free. Required if turnover crosses the VAT threshold.' },
      { category: 'Accountant', min: 600, max: 3000, note: 'Annual cost; varies by complexity.' },
      { category: 'Business Insurance', min: 200, max: 1500, note: 'Public liability and employer liability where applicable.' },
      { category: 'Office Space', min: 300, max: 5000, note: 'Monthly. London is significantly higher.' },
    ],
  },
  UAE: {
    currency: 'AED',
    ...
    items: [
      ...
      { category: 'Visa + Residency', min: 4000, max: 15000, note: 'Investor, partner, or employee visas depending on setup.' },
      { category: 'Local Sponsor / Agent', min: 0, max: 20000, note: 'Depends on mainland activity and current ownership rules.' },
      { category: 'Office / Flexi-desk', min: 2000, max: 10000, note: 'Flexi-desk in a free zone is often the cheapest option.' },
  ...
  Canada: {
    ...
    items: [
      ...
      { category: 'Business Licence', min: 100, max: 500, note: 'Municipal licence may be required.' },
      { category: 'HST/GST Registration', min: 0, max: 0, note: 'Free. Required after the revenue threshold.' },
      { category: 'Accountant', min: 800, max: 3500, note: 'Annual; varies by province and complexity.' },
      { category: 'Office Space', min: 500, max: 4000, note: 'Monthly. Toronto and Vancouver are usually highest.' },
    ],
  },
  Australia: {
    currency: 'AUD',
    ...
    items: [
      ...
      { category: 'Company Registration', min: 497, max: 497, note: 'ASIC company registration fee.' },
      { category: 'Business Name', min: 37, max: 88, note: 'ASIC fee for 1 or 3 years.' },
      { category: 'GST Registration', min: 0, max: 0, note: 'Free. Required after the revenue threshold.' },
      { category: 'Accountant', min: 1000, max: 4000, note: 'Annual; depends on business structure.' },
      { category: 'Office Space', min: 500, max: 5000, note: 'Monthly. Sydney and Melbourne are premium markets.' },
    ],
  },
}
      ...
      <div className="costs-hero">
        <div className="container">
          ...
          <p className="costs-hero__subtitle">
            Approximate costs for registering and running your business in another country, broken down by category.
          </p>
        </div>
      </div>
      ...
      <div className="container costs-body">
        <div className="costs-selectors">
          <div className="costs-selector-group">
            ...
            <select id="cost-country" className="input" value={country} onChange={e => setCountry(e.target.value)}>
              {COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="costs-selector-group">
            <label htmlFor="cost-business" className="costs-label">Business Type</label>
            <select id="cost-business" className="input" value={business} onChange={e => setBusiness(e.target.value)}>
              {['Restaurant / Food Business', 'IT Company / Tech Startup', 'Supermarket / Grocery Store', 'Salon / Beauty Business', 'Consulting Firm', 'E-commerce Store'].map(b => <option key={b} value={b}>{b}</option>)}
            </select>
          </div>
        </div>
          ...
          <div className="costs-table" role="table" aria-label={`Cost breakdown for ${country}`}>
            ...
            {data.items.map((item, i) => (
              <div key={i} className="costs-table__row" role="row">
                ...
                <span className="costs-table__amount costs-table__amount--min" role="cell">{item.min === 0 ? 'Free' : `${data.symbol}${item.min.toLocaleString()}`}</span>
                <span className="costs-table__amount costs-table__amount--max" role="cell">{item.max === 0 ? 'Free' : `${data.symbol}${item.max.toLocaleString()}`}</span>
            ...
            <div className="costs-table__total" role="row">
              ...
              <span className="costs-table__amount--min" role="cell"><strong>{data.symbol} {total.min.toLocaleString()}</strong></span>
              <span className="costs-table__amount--max" role="cell"><strong>{data.symbol} {total.max.toLocaleString()}</strong></span>
              <span role="cell" />
            </div>
          </div>
        </div>
        ...
        <div className="costs-disclaimer">
          <AlertCircle size={15} />
          <p>
            Figures are approximate estimates for reference only. Actual costs vary depending on business type, location, professional fees, and current government rules.
          </p>
        </div>
