...
import {
  ChevronDown, CheckCircle, Circle, Clock, DollarSign,
...
const LOCATIONS = ['Pakistan (Karachi)', 'Pakistan (Lahore)', 'Pakistan (Islamabad)', 'Pakistan (Other)', 'Already abroad']

/* Roadmap template — parametric based on country / business */
  ...
  return [
    {
      ...
      duration: '1–2 weeks',
      steps: [
        { title: 'Market Research',         desc: `Study demand for ${business} in ${country}. Analyse competitors, pricing, and target customers.`, done: false },
        { title: 'Business Plan',           desc: 'Write a basic business plan: objectives, revenue model, startup costs, break-even projection.',   done: false },
        { title: 'Visa / Right to Work',    desc: isUAE ? 'Apply for UAE Investor Visa or Entrepreneur Visa from a Pakistani embassy.' : `Check ${country} visa requirements for Pakistani nationals starting a business.`, done: false },
      ],
    },
    {
      ...
      steps: [
        { title: 'Choose Business Structure', desc: isUK ? 'Register as a Sole Trader or Private Limited Company (Ltd) via Companies House.' : isUS ? 'Decide between LLC, Corporation, or Sole Proprietorship based on your state.' : `Select the most appropriate legal entity type for ${country}.`, done: false },
        ...
        { title: 'Obtain Business Number',    desc: isUK ? 'Receive your Unique Taxpayer Reference (UTR) and Company Registration Number (CRN).' : isUS ? 'Apply for an Employer Identification Number (EIN) from the IRS — free and instant.' : `Register for the national business identification number in ${country}.`, done: false },
      ],
    },
    {
      ...
      steps: [
        ...
        { title: 'Health & Safety Certification', desc: 'Complete any mandatory safety inspections or certifications for your premises.', done: false },
        { title: 'Local Council Approval',    desc: 'Register with the local municipality or council and obtain any premises-specific permits.', done: false },
      ],
    },
    {
      ...
      duration: '1–2 weeks',
      steps: [
        { title: 'Open Business Bank Account', desc: `Open a dedicated business account with a local ${country} bank. Bring registration docs, passport, and proof of address.`, done: false },
        { title: 'Set Up Accounting',           desc: 'Choose accounting software (QuickBooks, Xero, or local equivalent) and register for VAT/Tax if threshold exceeded.', done: false },
        { title: 'Secure Initial Funding',       desc: 'Explore local grants, startup loans, or investor networks available in your region.', done: false },
      ],
    },
    {
      ...
      steps: [
        ...
        { title: 'Hire Staff',                desc: `Understand local employment laws in ${country} before hiring. Complete necessary payroll registration.`,   done: false },
        { title: 'Launch Marketing',          desc: 'Set up a Google Business Profile, social media pages, and announce your grand opening.',                    done: false },
      ],
    },
  ]
}
...
export default function Roadmap() {
  ...
  /* if country pre-filled from URL, keep it */
  useEffect(() => {
  ...
  const handleGenerate = (e) => {
    ...
    if (!location) { setError('Please enter your current location.'); return }
      ...
      <div className="roadmap-hero">
        <div className="container">
          ...
          <p className="roadmap-hero__subtitle">
            Tell us where you want to start your business and what kind of business — we'll generate a complete, step-by-step launch guide instantly.
          </p>
        </div>
      </div>
        ...
        {!generated && (
          <div className="roadmap-form-wrap">
            <form className="roadmap-form card" onSubmit={handleGenerate} noValidate aria-label="Roadmap generator form">
              ...
              <div className="roadmap-form__fields">
                ...
                <div className="roadmap-form__field">
                  <label htmlFor="current-location" className="roadmap-form__label">
                    Your Current Location <span aria-hidden="true">*</span>
                  </label>
                  <select
                    id="current-location"
                    className="input"
              ...
              <button
                type="submit"
                className="btn btn-primary btn-lg roadmap-form__submit"
                disabled={loading}
                aria-busy={loading}
              >
                {loading ? (
          ...
          <div className="roadmap-result anim-fade-up">
            ...
            <div className="roadmap-phases">
              {phases.map((phase, pi) => (
                <div
                  key={pi}
                  className={`roadmap-phase roadmap-phase--${phase.color}${openPhase === pi ? ' roadmap-phase--open' : ''}`}
                >
                  <button
                    className="roadmap-phase__header"
                  ...
                  >
                    ...
                    <div className="roadmap-phase__progress-mini">
                      <span>
                        {phase.steps.filter((_, si) => completed[`${pi}-${si}`]).length}/{phase.steps.length}
                      </span>
                    </div>
                    <ChevronDown
                      size={18}
                      className={`roadmap-phase__chevron${openPhase === pi ? ' roadmap-phase__chevron--open' : ''}`}
                      aria-hidden="true"
                    />
                  </button>
                    ...
                    <ul className="roadmap-steps" role="list">
                      {phase.steps.map((step, si) => {
                        ...
                        return (
                          <li
                            key={si}
                            className={`roadmap-step${done ? ' roadmap-step--done' : ''}`}
                          >
                            <button
                              className="roadmap-step__check"
                              onClick={() => toggleStep(pi, si)}
                              aria-label={done ? `Mark "${step.title}" as incomplete` : `Mark "${step.title}" as complete`}
                              aria-pressed={done}
                            >
                              {done
                                ? <CheckCircle size={22} />
